    ...
    for pencil in pencils:
        if pencil.name in gp_HoldFor_map.keys():
            if gp_HoldFor_map[pencil.name][0]:
                ...
                new_non_selected_kf_x = np.zeros(len(non_selected_kf_x), dtype=np.int32)
                ...
                if len(non_selected_kf_x) > 0:
                    # Check if non_selected is before or after marker
                    markerBeforeNonSelected = True if non_selected_kf_x[0] > timeline_marker else False
                    ...
                    new_non_selected_kf_x = np.round( non_selected_kf_x + select_non_select_distance + initial_distance ).astype(int)
                ...
                for layer in layers:
                    if not layer.lock:
                        ...
                        if len(non_selected_kf_x) > 0:
                            print(f"marker is before Non selected {markerBeforeNonSelected}")
                            if not markerBeforeNonSelected: 
                                for keyframe in keyframes:
                                    if keyframe.frame_number in non_selected_kf_x:
                                        kf_ix = int(np.where(non_selected_kf_x == keyframe.frame_number)[0])
                                        # print(f"{kf_ix} | {new_non_selected_kf_x[kf_ix]}")
...
class GP_SpreadTo_Operator(bpy.types.Operator):
        bl_idname = "scene.kfn_gp_spreadto"
        bl_label = "GP HoldFor"
