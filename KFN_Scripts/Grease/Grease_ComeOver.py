...
def gp_come_over():
        ...
        for layer in layers:
            ...
            if len(keyframes_selected) > 0:
                ...
                for keyframe in keyframes_selected:
                    gp_ComeOver_map[name][0].append(keyframe)
                    selected_keyframes.append(keyframe)

                for keyframe in keyframes_non_selected:
                    gp_ComeOver_map[name][1].append(keyframe)
    ...
    for pencil in pencils:
        ...
        if pencil.name in gp_ComeOver_map.keys():
            keys_in_Map = gp_ComeOver_map[pencil.name]
